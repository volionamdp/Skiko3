...
) : StickerModel(uuid, standardSize, centerPoint, rotate, scale) {
    override fun setData(stickerModel: StickerModel) {
        setCenterPoint(stickerModel.getCenterX(), stickerModel.getCenterY())
        rotate = stickerModel.rotate
        scale = stickerModel.scale
        if (stickerModel is TextModel) {
            textString = stickerModel.textString
            textFont = stickerModel.textFont
            textColor = stickerModel.textColor
            textOpacityColor = stickerModel.textOpacityColor
            ...
            backgroundOpacityColor = stickerModel.backgroundOpacityColor
            ...
            textBold = stickerModel.textBold
            textItalic = stickerModel.textItalic
            textUnderLine = stickerModel.textUnderLine
            textAlign = stickerModel.textAlign

        }
    }

    fun generatorKeyFont(): String {
        return textFont
    }

    fun generatorTextKey(): String {
        return "${generatorKeyFont()}$textSize$textLineSpacing$textAlign$textString"
    }
