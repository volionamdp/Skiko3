...
class AudioTransCoder {
    ...
    private final BlockingQueue<RawBuffer> mRawQueue = new LinkedBlockingQueue<>(10);

    private File mInputFile;
    private File mOutputFile;
    private long mStartPosMs;
    private long mEndPosMs;
    private long mDurationVideoMs;
    ...
    public static class Builder {
        private final AudioTransCoder transCodeAction = new AudioTransCoder();
        ...
        public Builder startTime(long startTimeMs) {
            transCodeAction.mStartPosMs = startTimeMs;
            return this;
        }

        public Builder videoDuration(long durationMs) {
            transCodeAction.mDurationVideoMs = durationMs;
            return this;
        }

        public Builder endTime(long endTimeMs) {
            transCodeAction.mEndPosMs = endTimeMs;
            return this;
        }
    ...
    private class DecodeInputWorker extends Thread {
        ...
        private void decodeInput21() {
            ...
            long timeStamp;
            int loopCount = 0;
            long duration = (mEndPosMs - mStartPosMs) * 1000;
            while (true) {
                ...
                if (inputBufferId >= 0) {
                    ...
                    if (loopCount * duration + (timeStamp - mStartPosMs * 1000) >= mDurationVideoMs * 1000) {
                        sampleSize = -1;
                    }
                    if (sampleSize != -1 && timeStamp >= mEndPosMs * 1000) {
                        loopCount++;
                        extractor.seekTo(mStartPosMs * 1000, MediaExtractor.SEEK_TO_CLOSEST_SYNC);
                        continue;
                    }
                    Log.d(TAG, "decodeInput21: " + timeStamp + "  " + loopCount);
