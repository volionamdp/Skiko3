...
final class BitmapUtils {
    ...
    public static Bitmap cropBitmapObjectWithScale2(
            Bitmap bitmap,
            int srcWidth,
            int srcHeight,
            float[] points,
        ...
        // get the rectangle in original image that contains the required cropped area (larger for non
        // rectangular crop)
        float scaleX = (float) bitmap.getWidth()/srcWidth;
        float scaleY = (float) bitmap.getHeight()/srcHeight;
        Rect rect =
                getRectFromPoints(
                        points,
                        srcWidth,
        ...
        if (rect.right > bitmap.getWidth()) rect.right = bitmap.getWidth();
        if (rect.top < 0) rect.top = 0;
        if (rect.bottom > bitmap.getHeight()) rect.bottom = bitmap.getHeight();

        // crop and rotate the cropped image in one operation
        Matrix matrix = new Matrix();
        matrix.setRotate(degreesRotated, bitmap.getWidth() / 2f, bitmap.getHeight() / 2f);
        matrix.postScale(flipHorizontally ? -scaleX : scaleX, flipVertically ? -scaleY : scaleY);
        Bitmap result =
                Bitmap.createBitmap(bitmap, rect.left, rect.top, rect.width(), rect.height(), matrix, true);
